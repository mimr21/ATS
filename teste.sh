find -name *.java -not -path "./__MACOSX/*"| uniq --check-chars 4 | sed  's/src.*/src/' | sed 's/\/\w*.java$//'| sed 's/\.\///' > output.txt
while read -r line
do
	PP=$(echo "$line" | cut -c -2)
	cd $PP
	mkdir -p src/main/java
	CAM=$(find -name *.java  -not -path "./__MACOSX/*" | uniq --check-chars 4 | sed  's/src.*/src/' | sed 's/\/\w*.java$//'| sed 's/\.\/[0-9]*//' | xargs -0) 
