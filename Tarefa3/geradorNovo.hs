...
genTransportadora' (a:as) (b:bs) (x:xs) (y:ys) (n:ns) (r:rn) (p:pn) = if (elem a as) then genTransportadora' as bs xs ys ns rn pn 
                                                                                     else [("Transportadora:"++"t"++a++","++b++","++(show x)++","++(show y)++","++n++","++r++","++(show p))]++(genTransportadora' as bs xs ys ns rn pn)
...
genUti' [] _ _ _ = []
genUti' (a:as) (d:ds) (x:xs) (y:ys) = if(elem a as) then genUti' as ds xs ys else [("Utilizador:"++"u"++a++","++d++","++(show x)++","++(show y))]++(genUti' as ds xs ys)
...
genVol' [] _ _ _ _ = []
genVol' (a:as) (d:ds) (x:xs) (y:ys) (t:ts) = if(elem a as) then genVol' as ds xs ys ts else [("Voluntarios:"++"v"++a++","++d++","++(show x)++","++(show y)++","++(show t))]++(genVol' as ds xs ys ts)
...
genLoja' [] _ _ _ = []
genLoja' (a:as) (b:bs) (x:xs) (y:ys)= if (elem a as) then genLoja' as bs xs ys else [("Loja:"++"l"++a++","++b++","++(show x)++","++(show y))]++(genLoja' as bs xs ys)
...
genEncomendas' _ _ _ [] _ = []
genEncomendas' _ _ _ _ [] = []
genEncomendas' (a:as) (u:us) (l:ls) (pe:pes) (p:ps) = if (elem a as) then genEncomendas' as us ls pes ps
                                                                     else [("Encomendas:"++"e"++a ++","++"u"++u++","++"l"++l++","++(show pe)++","++p)] ++(genEncomendas' as us ls pes ps)
...
genProducts n a = do num <- elements [1..35]
                     quantidade <- vectorOf num $ choose((0.000001),(20.000000))
                     valor <- vectorOf num $ choose((0.100000),(60.000000))
                     produtosRandom <- vectorOf num $ (elements produtos)
                     s <-(genProducts (n-1) a)
                     return ((++)([concatE(genP' (codIndex produtosRandom) produtosRandom quantidade valor)]) s  )

genP' :: [Codigo] -> [String] -> [Float] -> [Float] -> [String]
genP' [] _ _ _ = []
genP' (a:as) (b:bs) (c:cs) (d:ds) = if (elem a as) then genP' as bs cs ds else ["p"++a++","++b++","++(show c)++","++(show d)]++(genP' as bs cs ds)


codIndex :: [String] -> [Codigo]
codIndex [] = []
codIndex (h:t) = if (codIndex_aux h produtos 0 == " ") then codIndex t 
                                                   else codIndex_aux h produtos 0 : codIndex t 


codIndex_aux :: String -> [String] -> Int -> Codigo
codIndex_aux _ [] _ = " "
codIndex_aux x (h:t) n = if (x==h) then show(n) else codIndex_aux x t (n+1)
