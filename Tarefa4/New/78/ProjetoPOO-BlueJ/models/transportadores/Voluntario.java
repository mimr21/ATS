...
public abstract class Voluntario implements Serializable, ITransportadores {
    ...
    public Map<String, PedidoVoluntario> getHistorico() {
        ...
        for(Map.Entry<String, PedidoVoluntario> e: this.historico.entrySet()){
            res.put(e.getKey(), (PedidoVoluntario) e.getValue().copyPedidoVoluntario(e.getValue()));
        }
        return res;
    }
    ...
    public void setHistorico(Map<String, PedidoVoluntario> historico) {
        ...
        for(Map.Entry<String, PedidoVoluntario> e: historico.entrySet()){
            this.historico.put(e.getKey(), (PedidoVoluntario) e.getValue().copyPedidoVoluntario(e.getValue()));
        }
    }
     ...
     */
    public IPedidosTransportadores ocupaVoluntario(PedidoLoja p){
        ...
        this.historico.put(v.getCodigoPedido(),(PedidoVoluntario) v.copyPedidoVoluntario(v));
        return v.copyPedidoVoluntario(v);
    }
    ...
    public boolean aceitaCaracteristicasEncomenda(PedidoLoja p){
        ...
        if(!this.disponivel) b= false;

        if(!this.encomendaAtual.equals("None")) b=  false;

        if(this.historico.get(p.getCodigoPedido())!=null) b=  false;

        GPS loja = p.getGpsLoja();
        if(this.cord.distancia(loja)>this.raio) b=  false;

        GPS utilizador = p.getGpsUtilizador();
        if(this.cord.distancia(utilizador)>this.raio) b=  false;
