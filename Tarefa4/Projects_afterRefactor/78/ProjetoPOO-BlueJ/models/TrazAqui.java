...
public class TrazAqui implements Serializable {
    ...
    //Map que guarda toda a informa��o sobre os perfis que existem no sistema
    //Quando existe uma tentativa de login � aqui que est� a informa��o para validar o login.
    //A chave do map � uma string, que corresponde ao email.
    ...
    private transient Map<String, ITransportadores> transportadores;

    //Map que guarda os pedidos de prontos para entrega aceites pela loja, sendo a chave do map uma String que corresponde ao
    //codigo do pedido que no nosso caso � o mesmo que o da encomenda.
    ...
    //Lista que contem os proximos codigos para ser gerados, o array contem informa��o da seguinte maneira:
    ...
    /**
     * Construtor por omiss�o de TrazAqui
     */
    public TrazAqui(){
     ...
     */
    public TrazAqui(Map<String,Utilizador> utilizadores, Map<String,Loja> lojas, Map<String, ITransportadores> transportadores, Map<String,PedidoCompleto> historico, List<Integer> codigos, Map<String,Perfil> perfis, Map<String,PedidoLoja> pedidosParaRecolha){
        this.lojas = new HashMap<>();
        for (Map.Entry<String,Loja> ml : lojas.entrySet()){
            this.lojas.put(ml.getKey(),ml.getValue().copyLoja(ml.getValue()));
        }

        this.utilizadores = new HashMap<>();
        for (Map.Entry<String,Utilizador> mu : utilizadores.entrySet()){
            this.utilizadores.put(mu.getKey(),mu.getValue().copyUtilizador(mu.getValue()));
        }

        this.transportadores = new HashMap<>();
        for (Map.Entry<String, ITransportadores> mt : transportadores.entrySet()){
            if(mt.getValue() instanceof TransportadoraCertificada)
                this.transportadores.put(mt.getKey(),((TransportadoraCertificada) mt.getValue()).copyTransportadoraCertificada(mt.getValue()));
            else if (mt.getValue() instanceof TransportadoraNormal)
                this.transportadores.put(mt.getKey(), ((TransportadoraNormal) mt.getValue()).copyTransportadoraNormal(mt.getValue()));
        }

        this.historico = new HashMap<>();
        for (Map.Entry<String,PedidoCompleto> mh : historico.entrySet()){
            this.historico.put(mh.getKey(),mh.getValue().copyPedidoCompleto(mh.getValue()));
        }
        ...
        for (Map.Entry<String,Perfil> mpf : perfis.entrySet()){
            this.perfisExistentes.put(mpf.getKey(),mpf.getValue().copyPerfil(mpf.getValue()));
        }

        this.pedidosParaRecolha = new HashMap<>();
        for (Map.Entry<String,PedidoLoja> mpl : pedidosParaRecolha.entrySet()){
            this.pedidosParaRecolha.put(mpl.getKey(),mpl.getValue().copyPedidoLoja(mpl.getValue()));
        }
    }

    /**
     * Metodo que � chamado quando uma se pretende pedir ao sistema para entregar uma encomenda.
     * Metodo responsav�l por verificar se alguem disponivel t�m as caracteristicas necessarias para entregar a encomenda.
     ...
     * @return boolean que indica se o pedido foi ou n�o aceite pelo sistema
     */
    public boolean atribuiEntregador(PedidoLoja p){
        ...
        if(aceita) this.pedidosParaRecolha.put(p.getCodigoPedido(),p.copyPedidoLoja(p));
    ...
    public Utilizador getUtilizador(String cod){  
        return this.utilizadores.get(cod).copyUtilizador(this.utilizadores.get(cod));
    }
    ...
    public Loja getLoja(String cod){  
        return this.lojas.get(cod).copyLoja(this.lojas.get(cod));
    }
    
    /**
     * Metodo responsavel por devolver um clone de um Transportador dado o seu codigo
     */
    public ITransportadores getTransportador(String cod){

        if(this.transportadores.get(cod) instanceof TransportadoraCertificada)
            return ((TransportadoraCertificada) this.transportadores.get(cod)).copyTransportadoraCertificada(this.transportadores.get(cod));
        else if (this.transportadores.get(cod) instanceof TransportadoraNormal)
            return ((TransportadoraNormal) this.transportadores.get(cod)).copyTransportadoraNormal(this.transportadores.get(cod));
        return null;
    }

    /**
     * Metodo responsav�l por colocar na respetiva loja o numero (indicado) de pessoas na fila espera.
     ...
     * @param loja loja á qual vai ser adicionado um produto.
     ...
     * @param posEnc posi��o indicada do pedido a ir buscar.
     * @param codLoja codigo da loja que chamou este metodo.
     * @return pedido da loja.
     * @throws EncomendaNotFoundException caso o pedido n�o exista.
     */
    public PedidoLoja getPedidoUtilizadorDaLoja(int posEnc, String codLoja) throws EncomendaNotFoundException {
        return this.lojas.get(codLoja).getPedidoUtilizador(posEnc).copyPedidoLoja(this.lojas.get(codLoja).getPedidoUtilizador(posEnc));
    }
    ...
    public List<String> getListPedidosLoja(String loja){
        ...
        }catch (NullPointerException q) { return null;}
    }
     ...
     * @param pos posi��o da fila de espera do pedido a retirar.
     */
    ...
    public List<Loja> getLojasDisponiveis(){
        return this.lojas.values().parallelStream().map(loja -> loja.copyLoja(loja)).collect(Collectors.toList());
    }
     ...
     * @throws CodigoNotFoundException caso a loja n�o exista no sistema, sendo assim o codigo da loja invalido.
     */
     ...
     */
    public void utilizadorToLoja(IEncomendas e){
        PedidoUtilizador p = new PedidoUtilizador(e.copyEncomenda(e),this.utilizadores.get(e.getCodUtilizador()).getGps().copyGPS(this.utilizadores.get(e.getCodUtilizador()).getGps()),LocalDateTime.now());
    ...
    /**
     * Metodo responsavel por devolver uma lista com as notifica��es de um utilizador.
     * @param codU codigo do utilizador para o qual vai ser apresentadas as notifica��es.
     * @return lista das notifica��es
     */
    ...
    /**
     * Metodo responsavel por limpar todas as notifica��es de um utilizador.
     * @param cod codigo do utilizador a limpar as notifica��es.
     */
     ...
     * @param cod codigo do transportador a aplicar a mudan�a
     ...
     * @param codUtilizador codigo do utilizador que pretende esta informa��o.
    ...
    public List<PedidoTransportadora> getPedidosTransportadorasPendentes(String codUtilizador) {
        try {
            return this.utilizadores.get (codUtilizador).getPedidosPendentes ().values ().parallelStream ().map (pedidoTransportadora -> (PedidoTransportadora) pedidoTransportadora.copyPedidoTransportadora (pedidoTransportadora)).collect (Collectors.toList ());
        }catch (NullPointerException a) {return null;}
    }
    ...
    public List<String> encomendaParaEntrega(String codTransp){
        return this.pedidosParaRecolha.values().parallelStream().filter(p->this.transportadores.get(codTransp).aceitaCaracteristicasEncomenda(p))
                .map(PedidoLoja::toString).collect(Collectors.toList());
    }
     ...
     * @return boolean com o resultado de o sistema aceitar ou nao essa inten�ao de entrega.
     ...
     */
    public boolean aceitaIntecaoDeEntrega(String codigoTransportador, String codEncomenda) throws EncomendaNotFoundException, CodigoNotFoundException, NoSuchAlgorithmException {
        boolean res = false;
        ...
        ITransportadores transportador = this.transportadores.get(codigoTransportador);
        ...
        if(transportador instanceof Transportadora){
            ...
            if(t.aceitaCaracteristicasEncomenda(p)){
                ...
                IPedidosTransportadores pt = t.ocupaTransportadora(p.copyPedidoLoja(p));
                this.utilizadores.get(p.getUtilizador()).addPedidosPendentes((PedidoTransportadora) ((PedidoTransportadora) pt).copyPedidoTransportadora(pt));
            ...
            if(transportador instanceof Voluntario){
                ...
                if(v.aceitaCaracteristicasEncomenda(p)){
                    res = true;
                    this.pedidosParaRecolha.remove(codEncomenda);
                    IPedidosTransportadores pt = v.ocupaVoluntario(p);
     ...
     * @param avaliacao a avalia��o a ser dada ao transportador (0-5)
     * @throws ValorErradoException caso a avalia��o nao seja valida ou seja nao respeite o farmato de 0-5
     */
    ...
    public void finalizaTransportadores(String cod){
        ...
        this.utilizadores.get(p.getUtilizador()).addHistorico(p.copyPedidoCompleto(p));
        this.historico.put(p.getCodigoPedido(),p.copyPedidoCompleto(p));
    }
     ...
     * @return  a lista com o historico da loja bem como na ultima possi��o do array o total faturado pela mesma
     * @throws CodigoNotFoundException caso o codigo fornecido na seja invalido.
     */
    public List<String> totalFaturadoLoja(String cod) {
        try {
     ...
     * @return  a lista com o historico da transportadora bem como na ultima possi��o do array o total faturado pela mesma
     * @throws CodigoNotFoundException caso o codigo fornecido na seja invalido.
     */
     ...
     * @return lista com a informa�ao do top10
     */
    public List<String> top10Utilizadores(){
        ...
        for (Utilizador u : this.utilizadores.values()){
            ...
            aux.computeIfAbsent(num,k -> new ArrayList<>());
    ...
    /**
     * Metodo responsavel por devolver a lista do top 10 transportadora em fun�ao do numero de km
    ...
    public List<String> top10Transportadoras(){
        ...
        for (ITransportadores t : this.transportadores.values()){
            if(t instanceof Transportadora){
                ...
                aux.computeIfAbsent(km, k-> new ArrayList<>());
    ...
    public Perfil getPerfil(String email) throws ValorErradoException{
        try {
            return this.perfisExistentes.get(email).copyPerfil(this.perfisExistentes.get(email));
        }catch (NullPointerException e){
    ...
    public void adicionaPerfil(Perfil p){
        this.perfisExistentes.put(p.getEmail(),p.copyPerfil(p));
    }
     ...
     */
    public void adicionaTransportador(ITransportadores t){
        if (t instanceof VoluntarioEspecial)
            this.transportadores.put(t.getCodigo(),((VoluntarioEspecial)t).copyVoluntarioEspecial(t));
    }
    ...
    public void adicionaUtilizador(Utilizador u){
        this.utilizadores.put(u.getCodUtilizador(),u.copyUtilizador(u));
    }
    ...
    public void adicionaLoja(Loja l){
        this.lojas.put(l.getCodigoLoja(),l.copyLoja(l));
    }
     ...
     */
    public void guardaEstado(String nomeFicheiro) throws IOException, NullPointerException{
        ObjectOutputStream oos = null;
        FileOutputStream fos = null;
        try
        {
            fos = new FileOutputStream(new File(nomeFicheiro));
